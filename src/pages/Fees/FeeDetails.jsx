  ...
  if (!fee) {
    return (
      <div className="text-center py-12">
        <h3 className="text-lg font-medium text-gray-900">Fee record not found</h3>
        <button
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-1">Complete fee information for {fee.studentId?.name || student?.name || 'Student'}</p>
        </div>
        <button
          onClick={() => navigate('/fees/show')}
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              ...
              <span className="font-medium text-gray-900">{fee.studentId?.name || student?.name || 'N/A'}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-gray-500">Roll Number</span>
              <span className="font-medium text-gray-900">{fee.studentId?.rollNumber || student?.rollNumber || 'N/A'}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-gray-500">Class</span>
              <span className="font-medium text-gray-900">{fee.studentId?.class || student?.class || 'N/A'}</span>
            </div>
            {(student || fee.studentId) && (
              <>
                <div className="flex justify-between">
                  <span className="text-sm text-gray-500">Phone</span>
                  <span className="font-medium text-gray-900">{fee.studentId?.phone || student?.phone || 'N/A'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-gray-500">Email</span>
                  <span className="font-medium text-gray-900">{fee.studentId?.email || student?.email || 'N/A'}</span>
                </div>
              </>
            )}
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium text-gray-900">₹{fee.amount?.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium text-red-600">₹{((fee.amount || 0) - (fee.paidAmount || 0)).toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium text-gray-900">{fee.dueDate ? new Date(fee.dueDate).toLocaleDateString('en-IN') : 'N/A'}</span>
            </div>
            {fee.paidDate && (
              <div className="flex justify-between">
                <span className="text-sm text-gray-500">Payment Date</span>
                <span className="font-medium text-gray-900">{new Date(fee.paidDate).toLocaleDateString('en-IN')}</span>
              </div>
            )}
            {fee.paymentMethod && (
              <div className="flex justify-between">
                <span className="text-sm text-gray-500">Payment Method</span>
                <span className="font-medium text-gray-900">{fee.paymentMethod}</span>
              </div>
            )}
            {fee.receiptNumber && (
              <div className="flex justify-between">
                <span className="text-sm text-gray-500">Receipt Number</span>
                <span className="font-medium text-gray-900">{fee.receiptNumber}</span>
              </div>
            )}
          </div>
        </div>
      ...
      {fee.remarks && (
        <div className="bg-white rounded-xl shadow-sm p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
            <HiDocumentText className="w-5 h-5 mr-2 text-purple-600" />
            Remarks
          </h3>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
          <HiLightningBolt className="w-5 h-5 mr-2 text-indigo-600" />
          Actions
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {((fee.amount || 0) - (fee.paidAmount || 0)) > 0 && (
            <button
              ...
              className="bg-green-500 text-white px-4 py-3 rounded-lg hover:bg-green-600 transition-colors duration-200 flex items-center justify-center space-x-2 disabled:opacity-50"
            >
          ...
          <button
            ...
            className="bg-blue-500 text-white px-4 py-3 rounded-lg hover:bg-blue-600 transition-colors duration-200 flex items-center justify-center space-x-2"
          >
          ...
          <button
            ...
            disabled={actionLoading}
            className="bg-yellow-500 text-white px-4 py-3 rounded-lg hover:bg-yellow-600 transition-colors duration-200 flex items-center justify-center space-x-2 disabled:opacity-50"
          >
            <HiChatAlt className="w-4 h-4" />
            <span>{actionLoading ? 'Sending...' : 'Send Reminder'}</span>
          </button>
          
          <button
            onClick={() => navigate(`/students/details/${fee.studentId?._id || fee.studentId}`)}
            className="bg-purple-500 text-white px-4 py-3 rounded-lg hover:bg-purple-600 transition-colors duration-200 flex items-center justify-center space-x-2"
          >
